...
public class ViewCommentsCommand implements Command {
	...
	public String exec(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 1. 전체 게시물 수량 구하기
		p.setTotalRecord(CommentsDAO.getTotalCount(nickname));
		p.setTotalPage();
		...
		//List<CommentsVO> listC = CommentsDAO.getList(p.getBegin(), p.getEnd());
		...
		List<CommentsVO> listC = CommentsDAO.getList(cpvo);
		
		// 데이터를 화면에 표시(출력)
		req.setAttribute("listC", listC); // 게시글 데이터
		req.setAttribute("pvo", p); // 페이지 관련 데이터
		return "view_commentsList.jsp";
	}
